...
static void xfrog_inum_cvt(struct xfrog *froggie, void *v5, void *v1)
{
...
{
	return ioctl(froggie->fd, XFS_IOC_INUMBERS, req);
}
	...
	struct xfs_inumbers_req	*req)
{
	...
	error = ioctl(froggie->fd, XFS_IOC_FSINUMBERS, &bulkreq);
	...
	struct xfs_inumbers_req	*req)
{
	...
	error = xfrog_inumbers5(froggie, req);
	if (error == 0 || (froggie->flags & XFROG_FLAG_BULKSTAT_FORCE_V5))
		return 0;
...
try_v1:
	return xfrog_inumbers1(froggie, req);
}
