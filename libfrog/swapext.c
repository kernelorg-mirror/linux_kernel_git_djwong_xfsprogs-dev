	...
	struct file_swap_range	*req)
{
	...
	if (flags & FILE_SWAP_RANGE_FILE2_FRESH) {
		...
		/*
		 * stat the target file to get the inode number and use the
		 * latter to get the bulkstat info for the swapext cmd.
		 */
		error = fstat(dest->fd, &stat);
		if (error)
			return -errno;

		/* Now we need to get the target file's mtime and ctime. */
	...
	struct file_swap_range	*req)
{
	 ...
	 */
	if ((req->flags & FILE_SWAP_RANGE_ATOMIC) ||
	    !(req->flags & FILE_SWAP_RANGE_FULL_FILES) ||
	    !(req->flags & FILE_SWAP_RANGE_FILE2_FRESH))
		return -EOPNOTSUPP;

	/*
	 * Note that while the old interface contains a struct xfs_bstat, the
	 * kernel only ever checked bs_ctime and bs_mtime.
	 */
	...
	struct file_swap_range	*req)
{
	...
	if (error != -ENOTTY || (xfd->flags & XFROG_FLAG_SWAPEXT_FORCE_VFS))
		return error;
