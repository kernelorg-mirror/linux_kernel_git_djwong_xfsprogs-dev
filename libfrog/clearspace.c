 ...
 *
 * Second, the new FALLOC_FL_MAP_FREE_SPACE fallocate mode is used to map free
 * space in the target range to the balloon file.  This step will be repeated
 * after every space-clearing step below to capture that cleared space.
 * Concurrent writer threads will (hopefully) be allocated space outside the
 * target range.
 *
 * Freezing Used Space
 * -------------------
