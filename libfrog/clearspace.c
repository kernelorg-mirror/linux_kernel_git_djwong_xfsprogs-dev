	...
	const struct xfs_bulkstat	*bs)
{
	...
	handle_from_fshandle(&handle, req->fshandle, req->fshandle_len);
	...
	int			*fd)
{
	...
	*fd = -1;
	...
	target_fd = csp_open_by_handle(req, oflags, bulkstat);
	if (target_fd == -2)
		goto nothing_to_do;
	if (target_fd < 0)
		return -target_fd;
	...
	struct clearspace_req	**reqp)
{
	...
	*reqp = NULL;
	...
	if (req->fshandle)
		free_handle(req->fshandle, req->fshandle_len);
	free(req);
	return ret;
}
