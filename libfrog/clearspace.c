	...
	unsigned long long	length)
{
	...
	trace_fsmap(req, "dev 0x%x physical 0x%llx bytecount 0x%llx highkey 0x%llx",
			dev,
			(unsigned long long)physical,
			(unsigned long long)length,
	...
	struct clearspace_req	*req)
{
	...
	trace_fsmap(req,
 "ioctl dev 0x%x physical 0x%llx length 0x%llx highkey 0x%llx",
			mhead->fmh_keys[0].fmr_device,
	...
	unsigned long long	length)
{
	...
	trace_fsrefs(req, "dev 0x%x physical 0x%llx bytecount 0x%llx highkey 0x%llx",
			dev,
			(unsigned long long)physical,
			(unsigned long long)length,
