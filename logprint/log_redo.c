	...
	int			continued)
{
	...
	*ptr += src_len;
	...
	printf("(ino1: 0x%llx, ino2: 0x%llx, off1: %lld, off2: %lld, len: %lld, flags: 0x%llx)\n",
		(unsigned long long)ex->se_inode1,
		(unsigned long long)ex->se_inode2,
		(unsigned long long)ex->se_startoff1,
		(unsigned long long)ex->se_startoff2,
		(unsigned long long)ex->se_blockcount,
		(unsigned long long)ex->se_flags);
error:
	free(src_f);
	free(f);
	return error;
