...
fsrfs(char *mntdir, xfs_ino_t startino, int targetrange)
{
	...
	while ((ret = xfrog_bulkstat(&fsfrog, breq) == 0)) {
		...
		for (p = buf, endp = (buf + buflenout); p < endp ; p++) {
			...
			xfrog_bulkstat_to_bstat(&fsfrog, &bs1, p);
	...
	xfs_ino_t		ino)
{
	...
	req.hdr.ino = ino;
	error = xfrog_bulkstat_single(&fsfrog, &req);
	if (error < 0) {
	...
	xfrog_bulkstat_to_bstat(&fsfrog, &statbuf, &req.bulkstat);
	...
	struct xfs_bstat	*bstatp)
{
	...
	do {
		...
		if ((xfrog_bulkstat_single(&tfrog, &req)) < 0) {
			fsrprintf(_("unable to get bstat on temp file: %s\n"),
						strerror(errno));
			return -1;
		}
		xfrog_bulkstat_to_bstat(&tfrog, &tbstat, &req.bulkstat);
		if (dflag)
