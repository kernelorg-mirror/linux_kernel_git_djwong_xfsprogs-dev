	...
	struct fsxattr		*fsxp)
{
	 ...
	 * a bulkstat.
	 */
	error = xfrog_swapext_prep(file_fd,
			FILE_SWAP_RANGE_NONATOMIC | FILE_SWAP_RANGE_FULL_FILES,
			0, tfd, 0, statp->bs_size, &fsr);
	if (error) {
		fsrprintf(_("error %d setting up swapext request\n"), error);
		goto out;
	}
	xfrog_swapext_require_file2_fresh(&fsr, statp);
	...
	/* Swap the extents */
	error = xfrog_swapext(file_fd, &fsr);
	switch (error) {
		case 0:
			break;
	case ENOTSUP:
