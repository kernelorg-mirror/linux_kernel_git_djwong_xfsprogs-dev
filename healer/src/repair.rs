...
use crate::xfs_types::{XfsFid, XfsPeragNumber, XfsRtgroupNumber};
...
impl Repair {
    ...
    pub fn from_perag(t: XfsScrubType, group: XfsPeragNumber) -> Repair {
        Repair {
    ...
    pub fn from_rtgroup(t: XfsScrubType, group: XfsRtgroupNumber) -> Repair {
        Repair {
    ...
    fn outcome(&self) -> &str {
        ...
        if self.detail.sm_flags & xfs_fs::XFS_SCRUB_OFLAG_XFAIL != 0 {
            return "Seems correct but cross-referencing failed; offline repair recommended.";
        }

        if self.detail.sm_flags & xfs_fs::XFS_SCRUB_OFLAG_NO_REPAIR_NEEDED != 0 {
    ...
    fn report(&self, fh: &WeakHandle) {
        ...
        match self.group {
            ...
            RepairGroup::PerAg => {
                let agno: XfsPeragNumber = self.detail.into();
            ...
            RepairGroup::RtGroup => {
                let rgno: XfsRtgroupNumber = self.detail.into();
