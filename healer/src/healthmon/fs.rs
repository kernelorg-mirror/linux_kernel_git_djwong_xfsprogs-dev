...
pub enum XfsShutdownReason {
    #[strum(serialize = "corrupt_incore", to_string = "corrupt in-memory")]
    CorruptInCore,

    #[strum(serialize = "corrupt_ondisk", to_string = "corrupt ondisk")]
    CorruptOnDisk,
...
impl XfsShutdownEvent {
    /// Create a new whole-fs event object
    pub fn new(reasons: EnumSet<XfsShutdownReason>, status: XfsHealthStatus) -> XfsShutdownEvent {
        XfsShutdownEvent { reasons, status }
    }
}
...
impl XfsHealthEvent for XfsShutdownEvent {
    ...
    fn format(&self, _: &WeakHandle) -> (Option<PathBuf>, String) {
        (
            None,
            format!("{} {}", format_set(self.reasons), self.status),
        )
    }
}
