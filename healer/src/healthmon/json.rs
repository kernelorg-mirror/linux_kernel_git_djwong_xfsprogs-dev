    ...
    ($enum_type:ty , $err_msg:literal) => {
        impl $enum_type {
            ...
            pub fn try_set_from(
                ...
                let array = v.as_array().ok_or(baddata!("Not an array", $enum_type, v))?;
                let mut set = enumset::EnumSet::new();

                for jsvalue in array {
                    let value = jsvalue.as_str().ok_or(baddata!("Not a string", $enum_type, jsvalue))?;
    ...
    ($enum_type:ty , $err_msg:literal) => {
        impl TryFrom<&serde_json::Value> for $enum_type {
            ...
            fn try_from(v: &serde_json::Value) -> std::io::Result<$enum_type> {
                let value = v.as_str().ok_or(baddata!("Not a string", $enum_type, v))?;
...
impl TryFrom<&Value> for XfsAgNumber {
    ...
    fn try_from(v: &Value) -> Result<Self> {
        let m = v
            .as_u64()
            .ok_or(baddata!("AG number must be integer", Self, v))?;
...
impl TryFrom<&Value> for XfsRgNumber {
    ...
    fn try_from(v: &Value) -> Result<Self> {
        let m = v
            .as_u64()
            .ok_or(baddata!("rtgroup number must be integer", Self, v))?;
...
fn to_fid(ino: &Value, gen: &Value) -> Result<XfsFid> {
    let i = ino
        .as_u64()
        .ok_or(baddata!("inode number must be integer", XfsFid, ino))?;
    let g = gen
        .as_u64()
        .ok_or(baddata!("inode generation must be integer", XfsFid, gen))?;
...
fn to_range(pos: &Value, len: &Value) -> Result<XfsFileRange> {
    let p = pos
        .as_u64()
        .ok_or(baddata!("file position must be integer", XfsFileRange, pos))?;
    let l = len
        .as_u64()
        .ok_or(baddata!("file length must be integer", XfsFileRange, len))?;
...
fn shutdown_event_from_json(v: Value) -> Result<Box<dyn XfsHealthEvent>> {
    Ok(Box::new(XfsShutdownEvent::new(XfsShutdownReason::try_set_from(&v["reasons"])?)))
}
...
fn to_phys(dev: &Value, daddr: &Value, bbcount: &Value) -> Result<XfsPhysRange> {
    let a = daddr
        .as_u64()
        .ok_or(baddata!("daddr must be integer", XfsPhysRange, daddr))?;
    let b = bbcount
        .as_u64()
        .ok_or(baddata!("bbcount must be integer", XfsPhysRange, bbcount))?;
...
fn lost_event_from_json(v: Value) -> Result<Box<dyn XfsHealthEvent>> {
    let r = &v["count"];
    let count = r.as_u64().ok_or(baddata!("Not a count", LostEvent, r))?;
