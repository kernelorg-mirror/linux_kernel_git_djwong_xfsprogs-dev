...
macro_rules! enum_set_from_mask {
    ($enum_type:ty , { $($a:ident => $b:ident,)+ } ) => {
        impl $enum_type {
            ...
            pub fn from_mask(mask: u32) -> enumset::EnumSet<$enum_type> {
    ...
    ($enum_type:ty , { $($a:ident => $b:ident,)+ } ) => {
        impl $enum_type {
            ...
            pub fn from_value(value: u32) -> std::io::Result<$enum_type> {
                ...
                Err(baddata!("Unknown value", value))
            }
        }
    };
}
...
impl XfsHealthMonitor<'_> {
    ...
    pub fn try_new(fp: File, mountpoint: &Path, everything: bool) -> Result<XfsHealthMonitor> {
...
enum_set_from_mask!(XfsPeragMetadata, {
...
fn perag_event_from_cstruct(v: xfs_health_monitor_event) -> Result<Box<dyn XfsHealthEvent>> {
    ...
    Ok(Box::new(XfsPeragEvent::new(
        XfsPeragNumber::try_from(ge.gno as u64)?,
        XfsPeragMetadata::from_mask(ge.mask),
        XfsHealthStatus::from_value(v.type_)?,
    )))
}
...
fn rtgroup_event_from_cstruct(v: xfs_health_monitor_event) -> Result<Box<dyn XfsHealthEvent>> {
    ...
    Ok(Box::new(XfsRtgroupEvent::new(
        XfsRtgroupNumber::try_from(ge.gno as u64)?,
        XfsRtgroupMetadata::from_mask(ge.mask),
        XfsHealthStatus::from_value(v.type_)?,
    )))
}

enum_set_from_mask!(XfsRtgroupMetadata, {
...
enum_set_from_mask!(XfsInodeMetadata, {
...
fn inode_event_from_cstruct(v: xfs_health_monitor_event) -> Result<Box<dyn XfsHealthEvent>> {
    ...
    Ok(Box::new(XfsInodeEvent::new(
        XfsFid {
            ino: XfsInumber::try_from(ie.ino)?,
            gen: XfsIgeneration::try_from(ie.gen as u64)?,
        },
        XfsInodeMetadata::from_mask(ie.mask),
        XfsHealthStatus::from_value(v.type_)?,
    )))
}
...
enum_from_field!(XfsFileIoErrorType, {
    ...
    XFS_HEALTH_MONITOR_TYPE_DIOREAD  => DioRead,
    XFS_HEALTH_MONITOR_TYPE_DIOWRITE => DioWrite,
});
...
fn file_io_error_event_from_cstruct(
    ...
    Ok(Box::new(XfsFileIoErrorEvent::new(
        XfsFileIoErrorType::from_value(v.type_)?,
        XfsFid {
            ino: XfsInumber::try_from(fe.ino)?,
            gen: XfsIgeneration::try_from(fe.gen as u64)?,
        },
        XfsFileRange {
            pos: XfsPos::try_from(fe.pos)?,
            len: XfsIoLen::try_from(fe.len)?,
        },
    )))
}

enum_set_from_mask!(XfsWholeFsMetadata, {
...
fn wholefs_event_from_cstruct(v: xfs_health_monitor_event) -> Result<Box<dyn XfsHealthEvent>> {
    ...
    Ok(Box::new(XfsWholeFsEvent::new(
        XfsWholeFsMetadata::from_mask(fe.mask),
        XfsHealthStatus::from_value(v.type_)?,
    )))
}

enum_set_from_mask!(XfsShutdownReason, {
...
fn media_error_event_from_cstruct(v: xfs_health_monitor_event) -> Result<Box<dyn XfsHealthEvent>> {
    ...
    Ok(Box::new(XfsMediaErrorEvent::new(XfsPhysRange {
        device: XfsDevice::from_value(v.domain)?,
        daddr: me.daddr.into(),
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Err(e) = self.objiter.read_exact(&mut buf) {
            if e.kind() != ErrorKind::UnexpectedEof {
                eprintln!("{}: blob read error {}", self.mountpoint.display(), e);
            }
            return None;
        };
