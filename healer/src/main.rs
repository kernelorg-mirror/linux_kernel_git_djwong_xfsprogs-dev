...
use xfs_healer::healthmon::json::XfsHealthRawEvent as JsonEvent;
...
impl App {
    ...
    fn dispatch_json_event(
        ...
        threads.execute(move || {
            App::process_event(fh, should_log, should_repair, raw_event.cook());
        })
    }
    ...
    fn main(&mut self) -> Result<ExitCode> {
        ...
        if self.json {
            ...
            for raw_event in hmon {
                App::dispatch_json_event(&threads, fh.clone(), self.log, self.repair, raw_event);
            }
        } else {
            let hmon = CStructMonitor::try_new(fp, &self.path, self.everything)?;

            for raw_event in hmon {
                App::dispatch_cstruct_event(&threads, fh.clone(), self.log, self.repair, raw_event);
            }
        }
        threads.join();
