	...
	xfs_fileoff_t			block)
{
	...
	if (XFS_IS_CORRUPT(mp, block >= mp->m_sb.sb_rbmblocks)) {
		xfs_rtginode_mark_sick(args->rtg, XFS_RTG_BITMAP);
		return -EFSCORRUPTED;
	}

	return xfs_rtbuf_get(args, block, XFS_RTG_BITMAP);
}
	...
	xfs_fileoff_t			block)
{
	...
	if (XFS_IS_CORRUPT(mp, block >= mp->m_rsumblocks)) {
		xfs_rtginode_mark_sick(args->rtg, XFS_RTG_SUMMARY);
		return -EFSCORRUPTED;
	}
	return xfs_rtbuf_get(args, block, XFS_RTG_SUMMARY);
}
