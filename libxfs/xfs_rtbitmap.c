...
{
	if (xfs_has_rtgroups(mp))
		return le32_to_cpu((__le32)*wordptr);
...
{
	if (xfs_has_rtgroups(mp))
		*wordptr = (xfs_rtword_raw_t)cpu_to_le32(incore);
	else
		*wordptr = incore;
}
...
{
	xfs_rtword_raw_t *b;		/* current word in buffer */
...
xfs_suminfo_get(
	struct xfs_mount	*mp,
	xfs_suminfo_raw_t	*infoptr)
{
	if (xfs_has_rtgroups(mp))
		return be32_to_cpu((__be32)*infoptr);
	...
	xfs_suminfo_t	*sum)		/* out: summary info for this block */
{
	...
	xfs_suminfo_raw_t *sp;		/* pointer to returned data */
	...
	int		val)		/* 1 for free, 0 for allocated */
{
	...
	struct xfs_buf	*bp;		/* buf for the block */
	int		error;		/* error value */
	xfs_rtword_raw_t *first;		/* first used word in the buffer */
