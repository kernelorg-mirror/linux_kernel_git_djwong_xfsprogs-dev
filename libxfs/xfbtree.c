	...
	struct xfbtree		*xfbt)
{
	...
	libxfs_buftarg_free(xfbt->target);
	...
	struct xfbtree			**xfbtreep)
{
	...
	xfbt->target = libxfs_alloc_memory_buftarg(mp, xfbt->xfile);
	if (!xfbt->target) {
		error = -ENOMEM;
		goto err_xfile;
	}
	...
	int				*stat)
{
	 ...
	 */
	bt_xfoff = bitmap_take_first_set(xfbt->freespace, 0, -1ULL);
	if (bt_xfoff == -1ULL) {
