...
xfs_rtrefcountbt_absolute_maxlevels(void)
{
	unsigned int		minrecs[2];

	xfs_btree_absolute_minrecs(minrecs,
			XFS_BTREE_LONG_PTRS | XFS_BTREE_CRC_BLOCKS,
			sizeof(struct xfs_rtrefcount_rec),
			sizeof(struct xfs_rtrefcount_key) +
						sizeof(xfs_rtrefcount_ptr_t));
...
int __init
xfs_rtrefcountbt_create_cursor_cache(void)
{
	return xfs_btree_create_cursor_cache(XFS_BTNUM_RTREFC,
			"xfs_rtrefctbt_cur",
			xfs_rtrefcountbt_absolute_maxlevels());
}
