	...
	struct xfs_inode	*ip)
{
	...
	if (mp->m_rtrefc_maxlevels > XFS_BTREE_MAXLEVELS) {
		cur = kmem_zalloc(xfs_btree_cur_sizeof(mp->m_rtrefc_maxlevels),
				KM_NOFS);
...
xfs_rtrefcountbt_compute_maxlevels(
	struct xfs_mount		*mp)
{
	mp->m_rtrefc_maxlevels = xfs_btree_compute_maxlevels(mp->m_rtrefc_mnr,
			mp->m_sb.sb_rblocks);
}
	...
	xfs_filblks_t		*used)
{
	...
	*ask += xfs_rtrefcountbt_max_size(mp, mp->m_sb.sb_rblocks);
	*used += mp->m_rrefcountip->i_d.di_nblocks;
	...
	struct xfs_dinode	*dip)
{
	...
	if (xfs_rtrefcount_droot_space_calc(numrecs, level) > dsize)
		return -EFSCORRUPTED;
...
/*
 * Create a realtime refcount btree inode.  The caller must clean up @ic and
	...
	struct xfs_inode	**ipp)
{
	...
	*ipp = NULL;
	...
	error = xfs_imeta_create(tpp, &XFS_IMETA_RTREFCOUNTBT, S_IFREG, ipp,
			ic);
	if (error)
		return error;
