	...
	struct xmbuf		**xmbp)
{
	...
	xmb->target.bt_bdev = (dev_t)-1;
	xmb->target.bt_bdev_fd = -1;
	xmb->target.bt_xfile = xmb->xfile;
	xmb->target.flags = XFS_BUFTARG_MEM;
	xmb->target.bcache = cache;

	error = pthread_mutex_init(&xmb->target.lock, NULL);
	if (error)
		goto out_cache;
...
xmbuf_free(
	struct xmbuf		*xmb)
{
	ASSERT(xmb->target->flags & XFS_BUFTARG_MEM);

	cache_destroy(xmb->bcache);
	pthread_mutex_destroy(&xmb->target.lock);
	xfile_destroy(xmb->xfile);
	kfree(xmb);
}
