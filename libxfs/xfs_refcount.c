	...
	struct xfs_refcount_irec	*irec)
{
	if (cur->bc_btnum == XFS_BTNUM_RTREFC) {
		irec->rc_startblock = be32_to_cpu(rec->rtrefc.rc_startblock);
	...
	int				*stat)
{
	...
	BUILD_BUG_ON(XFS_REFC_COW_START != XFS_RTREFC_COW_START);
	...
	if (cur->bc_btnum == XFS_BTNUM_RTREFC) {
		...
		if (!xfs_verify_rgbext(rtg, realstart, irec->rc_blockcount))
			goto out_bad_rec;
	} else {
	...
	struct xfs_refcount_irec	*irec)
{
	...
	if (cur->bc_btnum == XFS_BTNUM_RTREFC) {
		rec.rtrefc.rc_startblock = cpu_to_be32(irec->rc_startblock);
	...
	struct xfs_btree_cur		**pcur)
{
	 ...
	 */
	rcur = *pcur;
	if (rcur != NULL && xfs_refcount_is_wrong_cursor(rcur, pag, rtg)) {
	...
	if (rcur == NULL) {
		if (ri->ri_realtime) {
			xfs_rtgroup_lock(tp, rtg, XFS_RTLOCK_REFCOUNT);
	...
	struct xfs_rtgroup		*rtg)
{
	...
	if (pag && mp->m_sb.sb_agblocks >= XFS_REFC_COW_START)
		return -EOPNOTSUPP;
	if (rtg && mp->m_sb.sb_rgblocks >= XFS_REFC_COW_START)
		return -EOPNOTSUPP;
	...
	list_for_each_entry_safe(rr, n, &debris, rr_list) {
		...
		if (rtg)
			fsb = xfs_rgbno_to_rtb(mp, rtg->rtg_rgno, agbno);
		else
