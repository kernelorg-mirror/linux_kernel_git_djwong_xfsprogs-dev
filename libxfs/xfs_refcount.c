	...
	struct xfs_refcount_irec	*rec)
{
	...
	if (xfs_btree_is_rtrefcount(cur->bc_ops)) {
		...
		fsbno = xfs_rgbno_to_rtb(cur->bc_ino.rtg, rec->rc_startblock);
	} else {
		fsbno = xfs_agbno_to_fsb(cur->bc_ag.pag, rec->rc_startblock);
	}
	...
	struct xfs_btree_cur		**pcur)
{
	...
	if (rcur == NULL) {
		...
		*pcur = rcur = xfs_rtrefcountbt_init_cursor(mp, tp, ri->ri_rtg,
				ri->ri_rtg->rtg_inodes[XFS_RTGI_REFCOUNT]);
	...
	struct xfs_rtgroup		*rtg)
{
	...
	list_for_each_entry_safe(rr, n, &debris, rr_list) {
		...
		if (rtg)
			fsb = xfs_rgbno_to_rtb(rtg, rr->rr_rrec.rc_startblock);
		else
