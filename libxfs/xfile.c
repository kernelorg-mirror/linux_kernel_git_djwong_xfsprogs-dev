...
#undef XFILE_MEMFD_FLAGS

#ifdef HAVE_MEMFD_CLOEXEC
# ifdef HAVE_MEMFD_NOEXEC_SEAL
#  define XFILE_MEMFD_FLAGS	(MFD_CLOEXEC | MFD_NOEXEC_SEAL)
# else
#  define XFILE_MEMFD_FLAGS	(MFD_CLOEXEC)
...
#ifdef XFILE_MEMFD_FLAGS
	/* memfd_create exists in kernel 3.17 (2014) and glibc 2.27 (2018). */
	fd = memfd_create(description, XFILE_MEMFD_FLAGS);
	if (fd >= 0)
		return fd;
#endif
