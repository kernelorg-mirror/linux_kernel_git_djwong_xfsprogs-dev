	...
	struct xfs_da_args	*args)
{
	 ...
	 */
	tablesize = xfs_attr3_leaf_tablesize(ichdr.count + 1, leaf);
	...
	int			mapindex)
{
	...
	new_end = xfs_attr3_leaf_tablesize(ichdr->count, leaf);
	...
	struct xfs_da_state_blk	*blk2)
{
	...
	if (count < ichdr1.count) {
		...
		max = ichdr2.firstused -
				xfs_attr3_leaf_tablesize(ichdr2.count, leaf1);
		if (space > max)
		...
		max = ichdr1.firstused -
				xfs_attr3_leaf_tablesize(ichdr1.count, leaf1);
		if (space > max)
	...
	int			*action)
{
	...
	bytes = xfs_attr3_leaf_tablesize(ichdr.count, leaf) + ichdr.usedbytes;
	...
	for (i = 0; i < 2; forward = !forward, i++) {
		...
		bytes = state->args->geo->blksize -
			...
			xfs_attr3_leaf_tablesize(ichdr.count + ichdr2.count,
					leaf);
	...
	struct xfs_da_args	*args)
{
	...
	ASSERT(args->index >= 0 && args->index < ichdr.count);
	ASSERT(ichdr.firstused >= xfs_attr3_leaf_tablesize(ichdr.count, leaf));
	 ...
	 */
	tablesize = xfs_attr3_leaf_tablesize(ichdr.count, leaf);
	 ...
	 */
	tmp = ichdr.usedbytes + xfs_attr3_leaf_tablesize(ichdr.count, leaf);
	...
	int				count)
{
	...
	ASSERT(ichdr_s->firstused >=
			xfs_attr3_leaf_tablesize(ichdr_s->count, leaf_s));
	ASSERT(ichdr_d->count < args->geo->blksize / 8);
	ASSERT(ichdr_d->firstused >=
			xfs_attr3_leaf_tablesize(ichdr_d->count, leaf_d));
	...
	for (i = 0; i < count; entry_s++, entry_d++, desti++, i++) {
		...
		if (entry_s->flags & XFS_ATTR_INCOMPLETE) { /* skip partials? */
			...
			ichdr_s->count -= 1;
			ichdr_d->count += 1;
			tmp = xfs_attr3_leaf_tablesize(ichdr_d->count, leaf_d);
