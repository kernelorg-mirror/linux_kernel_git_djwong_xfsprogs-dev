	...
	struct list_head		*b)
{
	...
	return sa->si_inode1->i_ino - sb->si_inode2->i_ino;
}
...
xfs_swapext_finish_item(
	...
	void				**state)
{
	struct xfs_swapext_intent	*swapext;
	int				error;

	swapext = container_of(item, struct xfs_swapext_intent, si_list);
	error = xfs_swapext_finish_one(tp, swapext->si_inode1,
			swapext->si_inode2, swapext->si_whichfork,
			&swapext->si_startoff1, &swapext->si_startoff2,
			&swapext->si_blockcount);
	/*
	 * If there's more work to do, we want to requeue ourselves after all
	 * other pending deferred operations have finished.  This includes all
	 * of the dfops that we queued directly as well as any new ones created
	 * in the process of finishing the others.  Doing so prevents us from
	 * queuing a large number of SXI log items in kernel memory, which in
	 * turn prevents us from pinning the tail of the log (while logging
	 * those new SXI items) until the first SXI items can be processed.
	 */
	if (!error && swapext->si_blockcount > 0)
		return -EMULTIHOP;
	...
	struct list_head		*item)
{
	...
	ip = swapext->si_inode1;
	error = xfs_defer_freezer_ijoin(freezer, ip);
	if (error)
		return error;
	swapext->si_inode1_ino = ip->i_ino;

	ip = swapext->si_inode2;
	error = xfs_defer_freezer_ijoin(freezer, ip);
	if (error)
		return error;
	swapext->si_inode2_ino = ip->i_ino;
	...
	struct list_head		*item)
{
	...
	ip = xfs_defer_freezer_igrab(freezer, swapext->si_inode1_ino);
	if (!ip)
		return -EFSCORRUPTED;
	swapext->si_inode1 = ip;

	ip = xfs_defer_freezer_igrab(freezer, swapext->si_inode2_ino);
	if (!ip)
		return -EFSCORRUPTED;
	swapext->si_inode2 = ip;
