...
setup_zones(void)
{
	...
	xfs_ili_zone = kmem_zone_init(sizeof(struct xfs_inode_log_item),
			"xfs_inode_log_item");
	xfs_buf_item_zone = kmem_zone_init(sizeof(struct xfs_buf_log_item),
			"xfs_buf_log_item");
	xfs_da_state_zone = kmem_zone_init(sizeof(struct xfs_da_state),
			"xfs_da_state");
	xfs_btree_cur_zone = kmem_zone_init(sizeof(struct xfs_btree_cur),
			"xfs_btree_cur");
	xfs_bmap_free_item_zone = kmem_zone_init(
			sizeof(struct xfs_extent_free_item),
			"xfs_bmap_free_item");
	xfs_trans_zone = kmem_zone_init(sizeof(struct xfs_trans), "xfs_trans");
	xfs_dir_startup();
...
/* Destroy all of the zone allocators we use. */
static int
destroy_zones(void)
{
	int	leaked = 0;
	...
	int		flags)
{
	...
	if (sbp->sb_imax_pct) {
		 ...
		 */
		mp->m_ino_geo.ig_maxicount = (sbp->sb_dblocks * sbp->sb_imax_pct) / 100;
		mp->m_ino_geo.ig_maxicount = XFS_FSB_TO_INO(mp,
				(mp->m_ino_geo.ig_maxicount / mp->m_ino_geo.ig_ialloc_blks) *
				 mp->m_ino_geo.ig_ialloc_blks);
	} else
		mp->m_ino_geo.ig_maxicount = 0;

	mp->m_ino_geo.ig_min_cluster_size = XFS_INODE_BIG_CLUSTER_SIZE;
	if (xfs_sb_version_hascrc(&mp->m_sb)) {
		int	new_size = mp->m_ino_geo.ig_min_cluster_size;

		new_size *= mp->m_sb.sb_inodesize / XFS_DINODE_MIN_SIZE;
		if (mp->m_sb.sb_inoalignmt >= XFS_B_TO_FSBT(mp, new_size))
			mp->m_ino_geo.ig_min_cluster_size = new_size;
	}
	mp->m_ino_geo.ig_blocks_per_cluster = xfs_icluster_size_fsb(mp);
	mp->m_ino_geo.ig_inodes_per_cluster = XFS_FSB_TO_INO(mp, mp->m_ino_geo.ig_blocks_per_cluster);
	mp->m_ino_geo.ig_cluster_align = xfs_ialloc_cluster_alignment(mp);
	mp->m_ino_geo.ig_cluster_align_inodes = XFS_FSB_TO_INO(mp, mp->m_ino_geo.ig_cluster_align);
	...
	if (xfs_sb_version_hasalign(&mp->m_sb) &&
	    ...
	    XFS_B_TO_FSBT(mp, mp->m_ino_geo.ig_min_cluster_size))
