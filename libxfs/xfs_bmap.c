	...
	int			whichfork)  /* data or attr fork */
{
	...
	xfs_bmap_add_free(cur->bc_tp, false, cbno, 1, &oinfo);
	ip->i_nblocks--;
	...
	int			bflags)	/* bmapi flags */
{
	...
	if (want_free) {
		if (xfs_is_reflink_inode(ip) && whichfork == XFS_DATA_FORK) {
			...
			__xfs_bmap_add_free(tp, isrt, del->br_startblock,
	...
	struct xfs_bmbt_irec	*mval)
{
	...
	if (isrt) {
		...
		error = xfs_rtallocate_extent(*tpp, rtx_in, 1, rtxlen_in,
		...
		if (args.fsbno == NULLFSBLOCK) {
			/*
			 * If extent allocation fails, need to roll the
			 * transaction to ensure that the AGFL fixup gets
			 * committed.
			 */
	...
	xfs_bmap_map_extent(*tpp, ip, XFS_DATA_FORK, mval);
	if (isrt)
		xfs_trans_mod_dquot_byino(*tpp, ip, XFS_TRANS_DQ_RTBCOUNT,
				mval->br_blockcount);
	else
