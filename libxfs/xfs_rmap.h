...
int xfs_rmap_scan_keyfill(struct xfs_btree_cur *cur, xfs_fsblock_t bno,
		xfs_filblks_t len, enum xfs_btree_keyfill *outcome);
...
int xfs_rmap_count_owners(struct xfs_btree_cur *cur, xfs_fsblock_t bno,
...
# ifdef CONFIG_XFS_RT
int xfs_rtrmap_hook_add(struct xfs_mount *mp, struct xfs_rmap_hook *hook);
void xfs_rtrmap_hook_del(struct xfs_mount *mp, struct xfs_rmap_hook *hook);
# endif /* CONFIG_XFS_RT */
