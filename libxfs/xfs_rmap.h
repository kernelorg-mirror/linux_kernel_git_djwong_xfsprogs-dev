...
int __xfs_rmap_finish_intent(struct xfs_btree_cur *rcur,
		enum xfs_rmap_intent_type op, xfs_agblock_t startblock,
		xfs_extlen_t blockcount, const struct xfs_owner_info *oinfo,
		bool unwritten);
...
# ifdef CONFIG_XFS_RT
int xfs_rgrmap_hook_add(struct xfs_rtgroup *rtg, struct xfs_rmap_hook *hook);
void xfs_rgrmap_hook_del(struct xfs_rtgroup *rtg, struct xfs_rmap_hook *hook);
# endif /* CONFIG_XFS_RT */
