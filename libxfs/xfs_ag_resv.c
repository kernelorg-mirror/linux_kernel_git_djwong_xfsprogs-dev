...
out:
	...
	/*
	 * If there isn't enough space in the AG to satisfy the reservation,
	 * let the caller know that there wasn't enough space.  Callers are
	 * responsible for deciding what to do next, since the AG can stumble
	 * along with insufficient reservation if data blocks are being freed
	 * to replenish the AG's free space.
	 */
	if (!error &&
