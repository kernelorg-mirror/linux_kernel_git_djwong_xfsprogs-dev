	...
	umode_t				mode)
{
	...
	error = xfs_dir_create_child(upd->tp, resblks, upd->dp, &xname,
			upd->ip);
	if (error)
		return error;
	...
	struct xfs_imeta_update		*upd)
{
	...
	error = xfs_dir_remove_child(upd->tp, resblks, upd->dp, &xname,
			upd->ip);
	if (error)
		return error;
	...
	struct xfs_imeta_update		*upd)
{
	...
	error = xfs_dir_add_child(upd->tp, resblks, upd->dp, &xname, upd->ip);
	if (error)
		return error;
...
/* Calculate the log block reservation to link a metadata inode. */
unsigned int
xfs_imeta_link_space_res(
	struct xfs_mount	*mp)
{
	return XFS_LINK_SPACE_RES(mp, MAXNAMELEN);
}
