	...
	struct xfs_inode	*ip)
{
	...
	ip->i_d.di_projid = 0;
	ip->i_d.di_flags |= (XFS_DIFLAG_IMMUTABLE | XFS_DIFLAG_SYNC |
	...
	struct xfs_inode	*ip)
{
	...
	return XFS_TEST_ERROR(fdblocks_under(ip, ip->i_meta_resv_asked / 10) ||
			      fdblocks_under(ip, btree_maxlevels),
			      mp, XFS_ERRTAG_IMETA_RESV_CRITICAL);
}
	...
	xfs_filblks_t		ask)
{
	...
	if (error) {
		trace_xfs_imeta_resv_init_error(mp, NULLAGNUMBER, error,
				_RET_IP_);
		return error;
	}
