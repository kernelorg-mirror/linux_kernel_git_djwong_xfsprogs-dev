...
{
	pptr->p_ino = dp->i_ino;
	pptr->p_gen = VFS_I(dp)->i_generation;
	pptr->p_namelen = name->len;
	memcpy(pptr->p_name, name->name, name->len);
}
	...
	struct xfs_parent_scratch	*scr)
{
	...
	xfs_parent_args_init(&scr->args, tp, &scr->rec, child, owner,
			pptr->p_name, pptr->p_namelen);
}
