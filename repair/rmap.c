	...
	struct xfs_rmap_irec	*rmap)
{
	...
	if (error) {
		do_log(_("agno 0x%x rmapbno 0x%x fsbcount 0x%x owner 0x%lx offset 0x%lx flags 0x%x err %d\n"),
				agno, rmap->rm_startblock, rmap->rm_blockcount,
				rmap->rm_owner, rmap->rm_offset,
				rmap->rm_flags, error);
