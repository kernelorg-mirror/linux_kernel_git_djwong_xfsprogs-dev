	...
	struct xfs_ag_rmap	*ag_rmap)
{
	...
	mp->m_rtrmap_maxlevels = libxfs_rtrmapbt_maxlevels_ondisk();

	error = xfile_create(mp, 0, "rtrmapbt repair", &xfile);
	if (error)
		goto nomem;
	...
	error = -libxfs_rtrmapbt_mem_create(mp, target, &ag_rmap->ar_xfbtree);
	if (error)
		goto nomem;
	...
	struct xfs_mount	*mp)
{
	...
	/* Initialize RT rmap data, then make it the -1st array element. */
	rmaps_init_rt(mp, &ag_rmaps[0]);
	ag_rmaps++;
	...
	struct xfs_mount	*mp)
{
	...
	/* Unhide the rt rmap data and free it. */
	ag_rmaps--;
	rmaps_destroy(mp, &ag_rmaps[0]);
	...
	struct rmap_mem_cur	*rmcur)
{
	...
	xfbt = rmap_for_ag(agno)->ar_xfbtree;
	...
	struct xfs_rmap_irec	*rmap)
{
	...
	if (error) {
		xfile_dump(xfbt->target->bt_xfile);
		do_error(_("adding rmap to in-memory btree\n"));
	}
	...
	uint64_t		owner)
{
	...
	assert(libxfs_verify_agbext(mp, agno, agbno, len));

	return slab_add(rmap_for_ag(agno)->ar_agbtree_rmaps, &rmap);
}
	...
	xfs_agnumber_t		agno)
{
	...
	/* Should never get called for the rt device. */
	if (agno == NULLAGNUMBER) {
		ASSERT(0);
		return;
	}
	...
	xfs_agnumber_t		agno)
{
	...
	struct xfs_trans	*tp;
	__be32			*agfl_bno, *b;
	struct xfs_ag_rmap	*ag_rmap = rmap_for_ag(agno);
	...
	xfs_agnumber_t		agno)
{
	...
	if (!xfs_has_reflink(mp))
		return 0;
	if (rmap_for_ag(agno)->ar_xfbtree == NULL)
		return 0;
	...
	xfs_agnumber_t		agno)
{
	...
	while ((error = -libxfs_btree_increment(rmcur.mcur, 0, &stat)) == 0) {
		if (!stat)
			break;
		nr++;
	}
	...
	int			*have)
{
	return -libxfs_rmap_lookup_le(bt_cur, rm_rec->rm_startblock,
			rm_rec->rm_owner, rm_rec->rm_offset, rm_rec->rm_flags,
			tmp, have);
}
...
_("Cannot iget realtime rmap inode 0x%llx, error %d.\n"),
		...
		/* Set m_rrmapip so the btree verifier confirms inode number */
		mp->m_rrmapip = ip;
...
_("Cannot iget realtime refcount inode 0x%llx, error %d.\n"),
		...
		/*
		 * Set m_rrefcountip so the btree verifier confirms inode
		 * number.
		 */
		mp->m_rrefcountip = ip;
