			...
			xfs_ino_t	*start_ino)
{
	...
	*start_ino = NULLFSINO;
	...
	ASSERT(mp->m_ino_geo.ig_ialloc_blks > 0);
	 ...
	 */
	if (mp->m_ino_geo.ig_ialloc_blks == 1)  {
		if (agbno > max_agbno)
			return 0;
		...
		*start_ino = XFS_AGINO_TO_INO(mp, agno, start_agino);
		...
		start_agbno = rounddown(XFS_INO_TO_AGBNO(mp, ino),
					fs_ino_alignment);
		end_agbno = start_agbno + mp->m_ino_geo.ig_ialloc_blks;
	 ...
	 */
	if (mp->m_ino_geo.ig_ialloc_blks > 1)  {
		if (agino > mp->m_ino_geo.ig_ialloc_inos)
			start_agbno = agbno - mp->m_ino_geo.ig_ialloc_blks + 1;
		else
			start_agbno = 1;
	}

	end_agbno = agbno + mp->m_ino_geo.ig_ialloc_blks;
	...
	if (irec_before_p)  {
		...
		if (irec_before_p->ino_startnum < agino)  {
			 ...
			 */
			if (irec_next_p != NULL &&
					agino + mp->m_ino_geo.ig_ialloc_inos >=
						irec_next_p->ino_startnum)
				end_agbno = XFS_AGINO_TO_AGBNO(mp,
						irec_next_p->ino_startnum);
		}
	 ...
	 */
	if (end_agbno - start_agbno < mp->m_ino_geo.ig_ialloc_blks)
		return(0);
	...
	if (num_blks < mp->m_ino_geo.ig_ialloc_blks || ino_cnt == 0)
		return 0;
	...
	if (num_blks % mp->m_ino_geo.ig_ialloc_blks != 0)  {
		num_blks = rounddown(num_blks, mp->m_ino_geo.ig_ialloc_blks);
	...
	int 			*bogus)
{
	...
	*bogus = 0;
	...
	blks_per_cluster = mp->m_ino_geo.ig_min_cluster_size >> mp->m_sb.sb_blocklog;
...
next_readbuf:
	...
	if (ino_discovery)  {
		for (;;)  {
			...
			if (icnt == mp->m_ino_geo.ig_ialloc_inos &&
	...
	_("would clear root inode %" PRIu64 "\n"),
				...
				if (!no_modify)  {
					do_warn(
	_("cleared metadata inode directory %" PRIu64 "\n"),
						ino);
				} else  {
					do_warn(
	_("would clear metadata inode directory %" PRIu64 "\n"),
						ino);
				}
	...
	int 			extra_attr_check)
{
	...
	while (ino_rec != NULL)  {
		...
		while (num_inos < mp->m_ino_geo.ig_ialloc_inos && ino_rec != NULL)  {
			/*
		...
		ASSERT(num_inos == mp->m_ino_geo.ig_ialloc_inos);
		...
		if (!bogus)
			...
			while (num_inos < mp->m_ino_geo.ig_ialloc_inos &&
					ino_rec != NULL)  {
...
process_uncertain_aginodes(xfs_mount_t *mp, xfs_agnumber_t agno)
{
	...
	fprintf(stderr, "ag inode = %d (0x%x)\n", agino, agino);
			 ...
			 */
			if (process_inode_chunk(mp, agno, mp->m_ino_geo.ig_ialloc_inos,
						nrec, 1, 0, 0, &bogus))  {
				/* XXX - i/o error, we've got a problem */
				abort();
			}
