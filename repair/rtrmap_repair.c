	...
	void			*priv)
{
	...
	memcpy(irec, rec, sizeof(struct xfs_rmap_irec));
	return 0;
}

/* Feed one of the new btree blocks to the bulk loader. */
STATIC int
xrep_rtrmap_alloc_block(
	struct xfs_btree_cur	*cur,
	...
	void			*priv)
{
	...
	return xrep_newbt_alloc_block(cur, &rr->new_fork_info, ptr);
}
...
xrep_rtrmap_btree_load(
	...
	struct xfs_btree_cur	**rtrmap_curp)
{
	struct xfs_btree_bload	rtrmap_bload = {
		.get_data	= xrep_rtrmap_get_data,
		.alloc_block	= xrep_rtrmap_alloc_block,
		.iroot_size	= xrep_rtrmap_iroot_size,
	};
	struct repair_ctx	*sc = rr->sc;
	struct xbtree_ifakeroot	*ifake = &rr->new_fork_info.ifake;
	int			error;

	estimate_inode_bload_slack(sc->mp, &rtrmap_bload);

	/* Compute how many blocks we'll need. */
	*rtrmap_curp = libxfs_rtrmapbt_stage_cursor(sc->mp, sc->tp, sc->ip,
			ifake);
	error = -libxfs_btree_bload_compute_geometry(*rtrmap_curp, &rtrmap_bload,
			rmap_record_count(sc->mp, NULLAGNUMBER));
	if (error)
		return error;
	libxfs_btree_del_cursor(*rtrmap_curp, error);
	*rtrmap_curp = NULL;
	 ...
	 */
	error = -libxfs_trans_reserve_more(sc->tp, rtrmap_bload.nr_blocks, 0);
	if (error)
		return error;
	 ...
	 * can't handle that.
	 */
	error = xrep_newbt_reserve_space(&rr->new_fork_info,
			rtrmap_bload.nr_blocks);
	if (error)
		return error;
	...
	*rtrmap_curp = libxfs_rtrmapbt_stage_cursor(sc->mp, sc->tp, sc->ip,
			ifake);
	error = -libxfs_btree_bload(*rtrmap_curp, &rtrmap_bload, rr);
	...
	struct xrep_rtrmap	*rr)
{
	...
	xrep_newbt_init_inode(&rr->new_fork_info, sc, XFS_DATA_FORK, &oinfo);
	cur = libxfs_bmbt_stage_cursor(sc->mp, sc->tp, sc->ip, ifake);
	 ...
	 */
	libxfs_rtrmapbt_commit_staged_btree(cur);
	...
	xrep_newbt_destroy(&rr->new_fork_info, error);
