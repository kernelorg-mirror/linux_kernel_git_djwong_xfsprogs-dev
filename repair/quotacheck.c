	...
	struct xfs_mount	*mp)
{
	...
	if (qc_has_quotafile(mp, XFS_DQTYPE_USER)) {
		user_dquots = qc_dquots_alloc(XFS_DQTYPE_USER);
		if (!user_dquots)
			goto err;
	...
	if (qc_has_quotafile(mp, XFS_DQTYPE_GROUP)) {
		group_dquots = qc_dquots_alloc(XFS_DQTYPE_GROUP);
		if (!group_dquots)
			goto err;
	...
	if (qc_has_quotafile(mp, XFS_DQTYPE_PROJ)) {
		proj_dquots = qc_dquots_alloc(XFS_DQTYPE_PROJ);
		if (!proj_dquots)
			goto err;
...
/* Purge all quotacheck records in a given cache and free the cache. */
static void
qc_freeall(
...
{
	qc_freeall(&user_dquots);
	qc_freeall(&group_dquots);
	qc_freeall(&proj_dquots);
}
