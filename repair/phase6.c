	...
	struct xfs_inode	*ip)
{
	...
	libxfs_icreate_args_rootfile(&args, ip->i_mount, mode);
	...
	struct xfs_imeta_update		*upd)
{
 ...
 _("couldn't create new rt metadata inode, error %d\n"), error);
	...
	*inop = NULLFSINO;
	...
	error = -libxfs_imeta_iget(mp, ino, XFS_DIR3_FT_REG_FILE, ipp);
	if (error)
		do_error(
	...
	struct xfs_mount	*mp)
{
	...
	i = -libxfs_parent_start(mp, &du.parent);
	if (i)
		do_error(_("%d - couldn't allocate parent pointer for %s\n"),
			i, ORPHANAGE);

	libxfs_icreate_args_rootfile(&args, mp, mode);
	...
	libxfs_irele(du.ip);
	libxfs_irele(du.dp);
	libxfs_parent_finish(mp, du.parent);
