	...
	void				*priv)
{
	...
	error = listxattr_walk_leaf_entries(ip, attr_fn, leaf_bp, priv);
	...
	void				*priv)
{
	...
	error = listxattr_find_leftmost_leaf(ip, seen_blocks, &leaf_bp);
	if (error)
		goto out_bitmap;

	for (;;) {
		error = listxattr_walk_leaf_entries(ip, attr_fn, leaf_bp,
				priv);
		if (error)
			goto out_leaf;
	...
	void			*priv)
{
	...
	if (ip->i_af.if_format == XFS_DINODE_FMT_LOCAL)
		return listxattr_walk_sf(ip, attr_fn, priv);
	...
	if (libxfs_attr_is_leaf(ip))
		return listxattr_walk_leaf(ip, attr_fn, priv);

	return listxattr_walk_node(ip, attr_fn, priv);
}
