...
_("invalid start block %llu in record %u of %s\n"),
		...
		if (len == 0 || !libxfs_verify_rtbno(mp, end - 1)) {
			do_warn(
...
_("%s btree block claimed (state %d), agno %d, bno %d, suspect %d\n"),
	...
	/* We're going to regenerate the rtrmap anyway, so free the blocks. */
	set_bmap(agno, agbno, XR_E_FREE);
	...
	const char			*name)
{
	...
	for (i = 0; i < numrecs; i++) {
		xfs_rtblock_t		b, rtb, end;
		xfs_filblks_t		len;
		xfs_nlink_t		nr;

		b = rtb = be64_to_cpu(rp[i].rc_startblock);
		len = be64_to_cpu(rp[i].rc_blockcount);
		nr = be32_to_cpu(rp[i].rc_refcount);
		if (b >= XFS_RTREFC_COW_START && nr != 1) {
			do_warn(
_("leftover rt CoW extent has incorrect refcount in record %u of %s\n"),
		...
		if (nr == 1) {
			if (rtb < XFS_RTREFC_COW_START) {
				do_warn(
_("leftover rt CoW extent has invalid startblock in record %u of %s\n"),
					i, name);
				suspect++;
			}
			rtb -= XFS_RTREFC_COW_START;
		}
		end = rtb + len;

		if (!libxfs_verify_rtbno(mp, rtb)) {
			do_warn(
_("invalid start block %lu in record %u of %s\n"),
					b, i, name);
			suspect++;
			continue;
		}
		if (len == 0 || end <= rtb ||
		    !libxfs_verify_rtbno(mp, end - 1)) {
			do_warn(
_("invalid length %lu in record %u of %s\n"),
					len, i, name);
			suspect++;
			continue;
		}
		...
		if (nr == 1) {
			...
			for (c = rtb; c < end; c++) {
				...
				case XR_E_COW:
					do_warn(
_("leftover rt CoW extent (%lu)\n"),
						c);
					suspect++;
					set_rtbmap(c, XR_E_FREE);
					break;
				default:
					do_warn(
_("extent (%lu) claimed, state is %d\n"),
						c, state);
					suspect++;
					break;
				}
...
_("invalid rt reference count %u in record %u of %s\n"),
		...
		if (b && b <= lastblock) {
			do_warn(_(
"out-of-order %s btree record %d (%lu %lu) in %s\n"),
					name, i, b, len, name);
			suspect++;
		} else {
			lastblock = end - 1;
...
_("%s btree block claimed (state %d), agno %d, agbno %d, suspect %d\n"),
	...
	_("bad btree pointer (%u) in %sbt block %u/%u\n"),
		...
		scan_lbtree(pbno, level, scan_rtrefcbt, type, whichfork, ino,
				tot, nex, blkmapp, bm_cursor, 0, check_dups,
				magic, refc_priv, &xfs_rtrefcountbt_buf_ops);
	}
out:
	if (suspect)
		refcount_avoid_check();
