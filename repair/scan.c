...
_("inode btree block claimed (state %d), agno %d, bno %d, suspect %d\n"),
	...
	if (level == 0) {
		...
		if (numrecs > mp->m_ino_geo.ig_inobt_mxr[0])  {
			numrecs = mp->m_ino_geo.ig_inobt_mxr[0];
			hdr_errors++;
		}
		if (isroot == 0 && numrecs < mp->m_ino_geo.ig_inobt_mnr[0])  {
			numrecs = mp->m_ino_geo.ig_inobt_mnr[0];
			hdr_errors++;
		}
	 ...
	 */
	if (numrecs > mp->m_ino_geo.ig_inobt_mxr[1])  {
		numrecs = mp->m_ino_geo.ig_inobt_mxr[1];
		hdr_errors++;
	}
	if (isroot == 0 && numrecs < mp->m_ino_geo.ig_inobt_mnr[1])  {
		numrecs = mp->m_ino_geo.ig_inobt_mnr[1];
		hdr_errors++;
	}
