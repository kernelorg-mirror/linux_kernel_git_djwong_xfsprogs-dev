	...
	struct xfs_sb		*new_sb)
{
	...
	if (!allow_rmapbt_upgrade_with_reflink && xfs_has_reflink(mp)) {
		printf(
	...
	struct xfs_sb		*new_sb)
{
	...
	new_sb->sb_features_incompat |= XFS_SB_FEAT_INCOMPAT_NEEDSREPAIR;
	new_sb->sb_features_log_incompat |= XFS_SB_FEAT_INCOMPAT_LOG_XATTRS;
	new_sb->sb_features_log_incompat |= XFS_SB_FEAT_INCOMPAT_LOG_EXCHMAPS;
	return true;
}
