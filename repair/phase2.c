	...
	struct xfs_sb		*new_sb)
{
	...
	if (xfs_has_realtime(mp) && mp->m_sb.sb_rextsize > 1) {
		printf(_("Reflink not supported for rt extent size > 1.\n"));
		exit(0);
	}
	...
	struct xfs_sb		*new_sb)
{
	...
	/* Try to size rtgroup between 1TB and 1 fsblock */
	for (i = 40; i > new_sb->sb_blocklog; i--) {
		unsigned int	rgsize = XFS_B_TO_FSBT(mp, 1ULL << i);

		rgsize -= rgsize % new_sb->sb_rextsize;
		new_sb->sb_rgextents = rgsize;
		if (new_sb->sb_rgextents >= XFS_MIN_RGEXTENTS &&
		    new_sb->sb_rgextents <=
				(XFS_MAX_RGBLOCKS / new_sb->sb_rextsize))
			break;
	}
	if (i < new_sb->sb_blocklog) {
		printf(
	_("Cannot determine rtgroup size between 1TB and %llu bytes.\n"),
				1ULL << i);
		exit(0);
	}
	new_sb->sb_rgcount = 0;
...
/*
 * Compute the amount of extra space needed for further rtrmap btree expansion.
 * If we're adding rtrmap to a filesystem then there won't be an rtrmap inode
 * and we'll return the full reservation charge.  If there's already an rtrmap
 * inode, then subtract the ondisk space usage because that's not reserved.
 */
static xfs_rfsblock_t
compute_rtrmap_reservation(
	struct xfs_rtgroup	*rtg)
{
	struct xfs_mount	*mp = rtg_mount(rtg);
	xfs_rfsblock_t		ret = libxfs_rtrmapbt_calc_reserves(mp);

	if (!xfs_has_rtrmapbt(mp))
		return 0;

	if (rtg_rmap(rtg))
		ret -= min(ret, rtg_rmap(rtg)->i_nblocks);
	return ret;
}
 ...
 _("Not enough free space to add realtime metadata files.\n"));
	...
	if (xfs_has_parent(mp) && !(old->features & XFS_FEAT_PARENT)) {
		if (mp->m_sb.sb_fdblocks < mp->m_sb.sb_dblocks / 4) {
			printf(
 _("Not enough free space to add parent pointers.\n"));
			exit(1);
		}
	}
