	...
	struct xfs_mount	*mp)
{
	...
	for (agno = 0; agno < mp->m_sb.sb_agcount; agno++) {
		free_slab(&pptrs[agno].pptr_recs);
		pthread_mutex_destroy(&pptrs[agno].lock);
	}
	free(pptrs);
	pptrs = NULL;
	...
	struct xfs_mount	*mp)
{
	...
	pptrs = calloc(mp->m_sb.sb_agcount, sizeof(struct ag_pptrs));
	if (!pptrs)
		do_error(
 _("init parent pointer per-AG record array failed: %s\n"),
	...
	for (agno = 0; agno < mp->m_sb.sb_agcount; agno++) {
		error = pthread_mutex_init(&pptrs[agno].lock, NULL);
		if (error)
			do_error(
 _("init agno %u parent pointer lock failed: %s\n"),
					agno, strerror(error));

		error = -init_slab(&pptrs[agno].pptr_recs,
	...
	struct xfs_inode	*dp)
{
	...
	error = -xfblob_store(names, &pptr.name_cookie, fname, pptr.namelen);
	...
	error = -slab_add(ag_pptrs->pptr_recs, &pptr);
	...
	struct ag_pptrs		*ag_pptrs)
{
	...
	error = -init_slab_cursor(ag_pptrs->garbage_xattr_recs, NULL, &cur);
	if (error)
 ...
 _("allocating %u bytes to remove ino %llu garbage xattr failed: %s\n"),
		...
		error = -xfblob_load(ag_pptrs->garbage_xattr_names,
 ...
 _("removing ino %llu garbage xattr failed: %s\n"),
	...
	free_slab(&ag_pptrs->garbage_xattr_recs);
	xfblob_destroy(ag_pptrs->garbage_xattr_names);
	ag_pptrs->garbage_xattr_names = NULL;
}
	...
	unsigned int		namelen)
{
	...
	if (no_modify) {
		do_warn(
 _("would delete garbage xattr in ino %llu\n"),
				(unsigned long long)ip->i_ino);
		return;
	}

	do_warn(
 _("deleting garbage xattr in ino %llu\n"),
			(unsigned long long)ip->i_ino);

	error = -init_slab(&ag_pptrs->garbage_xattr_recs,
	...
	error = -xfblob_create(mp, "garbage xattr names",
			&ag_pptrs->garbage_xattr_names);
	if (error)
		do_error("init garbage xattr names failed: %s\n",
				strerror(error));

stuffit:
	error = -xfblob_store(ag_pptrs->garbage_xattr_names,
	...
	error = -slab_add(ag_pptrs->garbage_xattr_recs, &garbage_xattr);
	if (error)
...
add_file_pptr(
	...
	unsigned char		*name)
{
	struct xfs_parent_name_rec pptr_rec = {
		.p_ino		= cpu_to_be64(ag_pptr->parent_ino),
		.p_gen		= cpu_to_be32(ag_pptr->parent_gen),
		.p_diroffset	= cpu_to_be32(ag_pptr->diroffset),
	};
	struct xfs_da_args	args = {
		.dp		= ip,
		.attr_filter	= XFS_ATTR_PARENT,
		.name		= (const unsigned char *)&pptr_rec,
		.namelen	= sizeof(pptr_rec),
		.value		= name,
		.valuelen	= ag_pptr->namelen,
	};

	return -libxfs_attr_set(&args);
}
...
{
	struct xfs_parent_name_rec pptr_rec = {
		.p_ino		= cpu_to_be64(file_pptr->parent_ino),
		.p_gen		= cpu_to_be32(file_pptr->parent_gen),
		.p_diroffset	= cpu_to_be32(file_pptr->diroffset),
	};
	struct xfs_da_args	args = {
		.dp		= ip,
		.attr_filter	= XFS_ATTR_PARENT,
	...
	struct ag_pptrs		*ag_pptrs)
{
	...
	error = -init_slab_cursor(ag_pptrs->file_pptr_recs, NULL, &cur);
	if (error)
	...
	struct ag_pptrs		*ag_pptrs)
{
	...
	if (!no_modify && ag_pptrs->have_garbage)
		remove_garbage_xattrs(ip, ag_pptrs);
