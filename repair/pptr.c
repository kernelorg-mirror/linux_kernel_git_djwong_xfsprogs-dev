	...
	struct file_scan	*fscan)
{
 ...
 _("loading garbage xattr value failed: %s\n"),
		...
		error = -libxfs_attr_removename(&args, true);
	...
	void			*priv)
{
	...
	ret = store_file_pptr_name(fscan, &file_pptr, &xname);
	if (ret)
		do_error(
 _("storing ino %llu parent pointer '%.*s' failed: %s\n"),
	...
	ret = -slab_add(fscan->file_pptr_recs, &file_pptr);
	if (ret)
	...
	const unsigned char		*name)
{
	...
	xfs_parent_rec_init(&pptr_rec, ag_pptr->parent_ino,
			ag_pptr->parent_gen);
	error = -libxfs_parent_lookup(NULL, ip, &xname, &pptr_rec, &scratch);
	if (!error || error != ENOATTR)
		return error;
