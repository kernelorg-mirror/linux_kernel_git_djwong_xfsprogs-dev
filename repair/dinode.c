...
_("indicated size of %s rtrmapbt root (%d bytes) greater than space in "
	...
	dmxr = libxfs_rtrmapbt_root_maxrecs(XFS_DFORK_SIZE(dip, mp, whichfork),
			false);
	pp = XFS_RTRMAP_ROOT_PTR_ADDR(dib, 1, dmxr);
...
_("cannot read inode %" PRIu64 ", file block %" PRIu64 ", disk block %" PRIu64 "\n"),
		...
		for (i = 0; i < dqperchunk; i++, dqid++, dqb++) {
			...
			if (libxfs_dquot_verify(mp, &dqb->dd_diskdq, dqid) != NULL ||
			    (dqb->dd_diskdq.d_type & XFS_DQTYPE_REC_MASK) != quota_type) {
