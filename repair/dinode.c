	...
	struct xfs_dinode	**dipp)
{
	 ...
	 */
	cluster_size = max(mp->m_ino_geo.ig_min_cluster_size, mp->m_sb.sb_blocksize);
	...
	cluster_blks = XFS_FSB_TO_DADDR(mp, max(1,
			mp->m_ino_geo.ig_min_cluster_size >> mp->m_sb.sb_blocklog));
...
_("indicated size of %s rtrmapbt root (%d bytes) greater than space in "
	...
	pp = XFS_RTRMAP_ROOT_PTR_ADDR(dib, 1, libxfs_rtrmapbt_maxrecs(
	...
	const char	*tag)
{
	if (*type != expected_type) {
		do_warn(
_("%s summary inode %" PRIu64 " has bad type 0x%x, "),
	...
	if (mp->m_sb.sb_rblocks == 0 && dinoc->di_nextents != 0)  {
		do_warn(
_("bad # of extents (%u) for %s inode %" PRIu64 "\n"),
			be32_to_cpu(dinoc->di_nextents), tag, lino);
		return 1;
	}
	return 0;
}
