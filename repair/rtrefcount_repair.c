	...
	struct xfs_btree_cur	*rtrmap_cur)
{
	...
	error = -libxfs_btree_bload_compute_geometry(rtrmap_cur,
			&rr->rtrefc_bload,
			refcount_record_count(sc->mp, NULLAGNUMBER));
	if (error)
		return error;
	...
	error = init_refcount_cursor(NULLAGNUMBER, &rr->slab_cursor);
	if (error)
		return error;
	...
	struct xrep_rtrefc	*rr)
{
	...
	cur = libxfs_rtrefcountbt_stage_cursor(sc->mp, sc->ip, ifake);
