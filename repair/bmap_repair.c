	...
	struct xrep_bmap	*rb)
{
	...
	struct xfs_btree_cur	*cur;
	struct xfs_inode	*ip;
	xfs_ino_t		rrmapino = get_rtrmap_ino();
	int			error;

	if (rrmapino == NULLFSINO)
		return -EIO;

	error = -libxfs_imeta_iget(mp, rrmapino, XFS_DIR3_FT_REG_FILE, &ip);
	if (error)
		return error;

	cur = libxfs_rtrmapbt_init_cursor(mp, sc->tp, ip);
	...
	uint64_t		nextents)
{
	...
	/* No delalloc extents to add to the new incore extent tree. */
	return 0;
}
