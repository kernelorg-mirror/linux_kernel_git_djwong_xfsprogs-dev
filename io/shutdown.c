...
"\n"
" -d -- Report against the data device (default).\n"
	...
	char			**argv)
{
	...
	while ((c = getopt(argc, argv, "dlr")) != EOF) {
		switch (c) {
		case 'd':
			me.flags = (me.flags & ~XFS_MEDIA_ERROR_DEVMASK) |
						XFS_MEDIA_ERROR_DATADEV;
			break;
		case 'l':
			me.flags = (me.flags & ~XFS_MEDIA_ERROR_DEVMASK) |
	...
	ret = ioctl(file->fd, XFS_IOC_REPORT_MEDIA_ERROR, &me);
	if (ret) {
		fprintf(stderr,
 "%s: ioctl(XFS_IOC_REPORT_MEDIA_ERROR) [\"%s\"]: %s\n",
