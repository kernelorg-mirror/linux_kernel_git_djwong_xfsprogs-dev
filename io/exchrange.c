	...
	char			**argv)
{
	...
	uint64_t		flags = XFS_EXCHRANGE_TO_EOF;
	...
	if (use_commit) {
		...
		ret = xfrog_commitrange_prep(&xcr, &xfd, dest_offset, fd,
				src_offset, length, &stat, NULL);
		if (!ret) {
		...
		xfrog_exchrange_prep(&fxr, dest_offset, fd, src_offset,
				length);
		ret = xfrog_exchrange(file->fd, &fxr, flags);
	}
	if (ret) {
		xfrog_perror(ret, "exchrange");
		exitcode = 1;
		goto out;
	}
	if (quiet_flag)
	...
	report_io_times("exchrange", &t2, dest_offset, length, length, 1,
			condensed);
out:
	close(fd);
	return 0;
...
static struct cmdinfo exchrange_cmd = {
	.name		= "exchrange",
	.cfunc		= exchrange_f,
	.argmin		= 1,
	.argmax		= -1,
	...
	char			*argv[])
{
	...
	*d = 0;
	 ...
	 */
	ret = xfrog_commitrange_prep(&p->xcr, &p->file_fd, 0, temp_fd, 0,
			stat.st_size, &stat, NULL);
	if (ret) {
		perror("update prep");
		goto fail;
	}
	...
	long long		*offset)
{
	...
	case FINISH_CHECK:
		...
		fallthrough;
	case FINISH_COMMIT:
		ret = xfrog_commitrange(p->file_fd.fd, &p->xcr, flags);
		if (ret) {
...
exchrange_init(void)
{
	exchrange_cmd.args = _("[-Ccfntw] [-d dest_offset] [-s src_offset] [-l length] <donorfile>");
	exchrange_cmd.oneline = _("Exchange contents between files.");

	add_command(&exchrange_cmd);
