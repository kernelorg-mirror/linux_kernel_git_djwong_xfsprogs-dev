	...
	char			**argv)
{
	...
	error = xfrog_prepare_geometry(&frog);
	if (error) {
	...
	switch (ver) {
	case 1:
		frog.flags |= XFROG_FLAG_BULKSTAT_FORCE_V1;
		break;
	case 5:
		frog.flags |= XFROG_FLAG_BULKSTAT_FORCE_V5;
		break;
	default:
		break;
	}

	while ((error = xfrog_bulkstat(&frog, breq)) == 0) {
		if (debug)
			printf(
	...
	char			**argv)
{
	...
	for (i = optind; i < argc; i++) {
		...
		error = xfrog_bulkstat_single(&frog, &breq);
		if (error) {
	...
	char			**argv)
{
	...
	while ((error = xfrog_inumbers(&frog, ireq)) == 0) {
		if (debug)
			printf(
_("bulkstat: startino=%"PRIu64" flags=0x%"PRIx32" agno=%"PRIu32" ret=%d icount=%"PRIu32" ocount=%"PRIu32"\n"),
				ireq->hdr.ino,
