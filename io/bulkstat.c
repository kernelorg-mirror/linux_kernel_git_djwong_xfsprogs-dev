	...
	char			**argv)
{
	...
	error = xfrog_geometry(file->fd, &fsgeom);
	if (error) {
		perror("xfrog_geometry");
		exitcode = 1;
		return 0;
	}
	...
	while ((error = xfrog_bulkstat(file->fd, &fsgeom, breq)) == 0) {
		if (debug)
			printf(
...
{
	struct xfs_fsop_geom	fsgeom = { 0 };
	struct xfs_bulkstat_single_req breq = { 0 };
	unsigned int		i;
	int			c;
	int			error;
	...
	for (i = optind; i < argc; i++) {
		...
		for (sm = tags; sm->tag; sm++) {
			if (!strcmp(argv[i], sm->tag)) {
				breq.hdr.ino = sm->code;
				breq.hdr.flags |= XFS_IREQ_SPECIAL;
				break;
			}
		}
		...
		if (sm->tag == NULL) {
			errno = 0;
			breq.hdr.ino = strtoull(argv[i], NULL, 10);
			if (errno) {
				perror(argv[i]);
				exitcode = 1;
				return 0;
		...
		error = xfrog_bulkstat_single(file->fd, &fsgeom, &breq);
		if (error) {
	...
	char			**argv)
{
	...
	while ((error = xfrog_inumbers(file->fd, &fsgeom, ireq)) == 0) {
		if (debug)
			printf(
_("bulkstat: startino=%"PRIu64" flags=0x%"PRIx32" agno=%"PRIu32" ret=%d icount=%"PRIu32" ocount=%"PRIu32"\n"),
				ireq->hdr.ino,
