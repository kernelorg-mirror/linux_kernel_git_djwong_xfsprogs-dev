	...
	char			*argv[])
{
	...
	*d = 0;
	 ...
	 * extent swap request.
	 */
	ret = xfrog_swapext_prep(&p->file_fd, FILE_SWAP_RANGE_COMMIT, 0,
			temp_fd, 0, stat.st_size, &p->swap_req);
	if (ret) {
		perror("update prep");
		goto fail;
	}
	...
	uint64_t		flags)
{
	...
	switch (how) {
	case FINISH_CHECK:
		p->swap_req.flags |= FILE_SWAP_RANGE_DRY_RUN;
		/* fall through */
	case FINISH_COMMIT:
	...
	char		*argv[])
{
	...
	uint64_t	flags = FILE_SWAP_RANGE_TO_EOF;
	int		c;

	while ((c = getopt(argc, argv, "hkn")) != -1) {
		switch (c) {
		case 'h':
			flags |= FILE_SWAP_RANGE_SKIP_FILE1_HOLES;
			break;
		case 'k':
