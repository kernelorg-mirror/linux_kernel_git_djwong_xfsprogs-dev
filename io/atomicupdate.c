	...
	char			*argv[])
{
	...
	*d = 0;
	...
	ret = xfrog_swapext_prep(&p->file_fd,
			FILE_SWAP_RANGE_TO_EOF | FILE_SWAP_RANGE_COMMIT,
			0, temp_fd, 0, 0, &p->swap_req);
	if (ret) {
		perror("update prep");
		goto fail;
	}
...
cancelupdate_f(
	...
	char		*argv[])
{
	return finish_update(FINISH_ABORT);
}
