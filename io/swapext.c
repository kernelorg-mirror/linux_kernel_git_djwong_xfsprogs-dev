...
" -f   -- Swap extents to the ends of both files, including the file sizes\n"
	...
	char			**argv)
{
	...
	struct stat		stat;
	uint64_t		flags = FILE_SWAP_RANGE_FILE2_FRESH |
	...
	while ((c = getopt(argc, argv, "ad:fl:s:u")) != -1) {
		switch (c) {
		case 'a':
			flags |= FILE_SWAP_RANGE_ATOMIC;
			break;
		case 'd':
		...
		case 'v':
			if (!strcmp(optarg, "xfsv0"))
				...
				fprintf(stderr, "version must be 0 or 5.\n");
				return 1;
			}
			break;
		default:
...
swapext_init(void)
{
	...
	swapext_cmd.args = _("[-a] [-f] [-u] [-d dest_offset] [-s src_offset] [-l length] [-v xfsv0|vfs] <donorfile>");
