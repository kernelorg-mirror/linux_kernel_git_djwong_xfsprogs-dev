	...
	struct cli_params	*cli)
{
	...
	case L_CONCURRENCY:
		if (!strcmp(value, "auto"))
			cli->log_concurrency = sysconf(_SC_NPROCESSORS_ONLN);
		else
			cli->log_concurrency = getnum(value, opts, subopt);
		break;
	default:
		return -EINVAL;
	}
	...
	struct xfs_mount	*mp)
{
	...
	if (cli->log_concurrency) {
		struct xfs_trans_res	res;

		libxfs_log_get_max_trans_res(&mount, &res);
		max_tx_bytes = (uint64_t)res.tr_logres * res.tr_logcount;
	}
	libxfs_umount(&mount);
...
_("external log device size %lld blocks too small, must be at least %lld blocks\n"),
	...
	if (!cfg->logblocks) {
		...
		if (cli->log_concurrency) {
			...
			/*
			 * Create a log that is large enough to handle
			 * simultaneous maximally sized transactions at the
			 * concurrency level specified by the user without
			 * blocking for space.  Increase the figure by 50% so
			 * that background threads can also run.
			 */
			log_bytes = max_tx_bytes * 3 * cli->log_concurrency / 2;
			min_logblocks = min(XFS_MAX_LOG_BYTES >> cfg->blocklog,
	...
	struct xfs_mount	*mp)
{
	...
	error = -libxfs_imeta_resv_init_inode(mp->m_rrmapip, ask);
	if (error)
	...
	error = -libxfs_imeta_resv_init_inode(mp->m_rrefcountip, ask);
	if (error)
		prealloc_fail(mp, error, ask, _("realtime refcount btree"));

	/* Unreserve the realtime metadata reservations. */
	libxfs_imeta_resv_free_inode(mp->m_rrefcountip);
	libxfs_imeta_resv_free_inode(mp->m_rrmapip);
