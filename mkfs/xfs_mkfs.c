...
_("rmapbt not supported on realtime devices without metadir feature\n"));
		...
		cli->sb_feat.zoned = false;
	}

	if (!cli->sb_feat.zoned) {
		if (cli->rtstart) {
			fprintf(stderr,
_("internal RT section only support in zoned mode\n"));
			usage();
		}
		if (cli->rtreserved) {
			fprintf(stderr,
_("reserved RT blocks only support in zoned mode\n"));
			usage();
		}
	}
	...
	char			**argv)
{
	...
	calculate_rtgroup_geometry(&cfg, &cli, &xi, &zt);
