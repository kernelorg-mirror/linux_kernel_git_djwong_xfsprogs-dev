...
struct sb_feat_args {
	...
	bool	rtsuper;		/* XFS_SB_FEAT_RO_COMPAT_RTSUPER */
...
/* realtime subvol */	[-r extsize=num,size=num,rtdev=xxx,rtgroups=0|1,\n\
			    rgcount=n,rgsize=n,rtsuper=0|1]\n\
	...
	struct cli_params	*cli)
{
	...
	case R_RGSIZE:
		...
		break;
	case R_RTSUPER:
		cli->sb_feat.rtsuper = getnum(value, opts, subopt);
		break;
	default:
		return -EINVAL;
	}
...
_("realtime groups not supported without exchange-range support\n"));
	 ...
	 * caller did not provide an explicit rtsuper parameter so that users
	 * can take advantage of labelled rt volumes.  It's not required for
	 * correct operation, but it costs us nothing to enable it.
	 */
	if (cli->sb_feat.rtgroups && !cli->sb_feat.rtsuper &&
	    !cli_opt_set(&ropts, R_RTSUPER)) {
		cli->sb_feat.rtsuper = true;
	}

	/* Realtime superblocks require realtime groups. */
	if (cli->sb_feat.rtsuper && !cli->sb_feat.rtgroups) {
		if (cli_opt_set(&ropts, R_RTSUPER)) {
			fprintf(stderr,
	...
	struct xfs_sb		*sbp)
{
	...
	if (fp->rtsuper)
		sbp->sb_features_ro_compat |= XFS_SB_FEAT_RO_COMPAT_RTSUPER;
	if (fp->rtgroups) {
	...
	struct xfs_mount	*mp)
{
  ...
  _("%s: couldn't grab buffers to write primary rt superblock\n"), progname);
	...
	if (error) {
		fprintf(stderr,
 _("%s: couldn't grab realtime superblock\n"), progname);
			exit(1);
	}
	...
	char			**argv)
{
 ...
 _("xfs dir/attr self-test failed, will not create a filesystem here.\n"));
	...
	if (xfs_has_rtsuper(mp) && cfg.rtblocks > 0)
		write_rtsuper(mp);
