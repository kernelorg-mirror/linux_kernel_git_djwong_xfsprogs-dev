	...
	const char		*tag)
{
	if (error == ENOSPC)
		fprintf(stderr,
	_("%s: cannot handle expansion of %s; need free %lu blocks, have %lu\n"),
				progname, tag, ask, mp->m_sb.sb_fdblocks);
	else
		fprintf(stderr,
	...
	struct xfs_mount	*mp)
{
	...
	for (agno = 0; 0 && agno < mp->m_sb.sb_agcount; agno++) {
		pag = libxfs_perag_get(mp, agno);
		error = -libxfs_ag_resv_init(pag, NULL);
		libxfs_perag_put(pag);
		if (error && error != ENOSPC) {
			fprintf(stderr,
	_("%s: error %d while checking free space for realtime metadata\n"),
					progname, error);
			exit(1);
		}
	}
	...
	error = -libxfs_imeta_resv_init_inode(mp, mp->m_rrefcountip, ask);
	if (error)
		prealloc_fail(mp, error, ask, _("realtime refcount btree"));

	/* Unreserve the realtime metadata reservations. */
	libxfs_imeta_resv_free_inode(mp, mp->m_rrefcountip);
