	...
	struct xfs_mount	*mp)
{
	...
	error = -libxfs_imeta_create_root(&upd);
	if (error)
		goto out_cancel;
	...
	mp->m_metadirip = upd.ip;
	return 0;

out_cancel:
	libxfs_imeta_cancel(&upd, error);
out_rele:
	if (ip)
		libxfs_irele(ip);
	return error;
	...
	struct xfs_inode	**ipp)
{
	 ...
	 * allocated.
	 */
	error = -libxfs_dialloc(tpp, 0, mode, &ino);
	if (error)
		return error;
	...
	struct xfs_rtgroup	*rtg)
{
	...
	if (xfs_has_rtsuper(mp) && rtg->rtg_rgno == 0) {
		 ...
		 * the data fork.
		 */
		cur = libxfs_rtrmapbt_init_cursor(mp, upd.tp, rtg, upd.ip);
