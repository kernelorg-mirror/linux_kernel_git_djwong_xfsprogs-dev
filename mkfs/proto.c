...
{
	ip->i_d.di_projid = fsx->fsx_projid;
	...
	struct xfs_inode	**ipp)
{
	...
	error = -libxfs_dir_ialloc(tpp, &args, ipp);
	if (error || dp)
		return error;

	/* Set root directory inode flags from the provided fsxattr. */
	ialloc_fsx_init(*tpp, *ipp, fsx);
	return 0;
}
...
#define	IF_FIFO		6
	...
	case IF_REGULAR:
		...
		error = ialloc(&tp, pip, mode | S_IFREG, 1, 0, &creds, fsxp,
				&ip);
		if (error)
	...
	case IF_BLOCK:
		...
		error = ialloc(&tp, pip, mode | S_IFBLK, 1,
	...
	case IF_CHAR:
		...
		majdev = getnum(getstr(pp), 0, 0, false);
		mindev = getnum(getstr(pp), 0, 0, false);
		error = ialloc(&tp, pip, mode | S_IFCHR, 1,
				IRIX_MKDEV(majdev, mindev), &creds, fsxp, &ip);
		if (error)
			fail(_("Inode allocation failed"), error);
		libxfs_trans_ijoin(tp, pip, 0);
	...
	case IF_FIFO:
		tp = getres(mp, 0);
		error = ialloc(&tp, pip, mode | S_IFIFO, 1, 0, &creds, fsxp,
				&ip);
		if (error)
			fail(_("Inode allocation failed"), error);
		libxfs_trans_ijoin(tp, pip, 0);
	...
	case IF_SYMLINK:
		...
		len = (int)strlen(buf);
		tp = getres(mp, XFS_B_TO_FSB(mp, len));
		error = ialloc(&tp, pip, mode | S_IFLNK, 1, 0, &creds, fsxp,
				&ip);
		if (error)
			fail(_("Inode allocation failed"), error);
	...
	case IF_DIRECTORY:
		tp = getres(mp, 0);
		error = ialloc(&tp, pip, mode | S_IFDIR, 1, 0, &creds, fsxp,
				&ip);
		if (error)
			fail(_("Inode allocation failed"), error);
	...
	struct xfs_mount	*mp)
{
	...
	error = -libxfs_imeta_create(&tp, &XFS_IMETA_METADIR, S_IFDIR, &ip,
			&ic);
	if (error)
		goto out_cancel;
	...
	struct xfs_mount	*mp)
{
	...
	error = -libxfs_imeta_create(&tp, &XFS_IMETA_RTBITMAP, S_IFREG,
			&rbmip, &ic);
	if (error) {
	...
	*(uint64_t *)&VFS_I(rbmip)->i_atime = 0;
	...
	error = -libxfs_imeta_create(&tp, &XFS_IMETA_RTSUMMARY, S_IFREG,
			&rsumip, &ic);
	if (error) {
	...
	if (mp->m_sb.sb_rbmblocks) {
		error = -libxfs_alloc_file_space(rbmip, 0,
	...
	_("Block allocation of the realtime bitmap inode failed"),
		 ...
		 */
		error = -libxfs_alloc_file_space(rsumip, 0, mp->m_rsumsize,
				XFS_BMAPI_ZERO);
		if (error)
			fail(
