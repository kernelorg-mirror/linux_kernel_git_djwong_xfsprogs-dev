...
#define	MKFS_BLOCKRES_INODE	\
	((uint)(mp->m_ino_geo.ig_ialloc_blks + (mp->m_ino_geo.ig_in_maxlevels - 1)))
...
/* Create a new metadata inode directory. */
static int
metadir_init(
	struct xfs_mount	*mp)
{
	...
	/*
	 * The metadata inode directory should always be part of the first
	 * inode chunk in the filesystem, and that should always exist, because
	 * we (re)create the root directory first.  So, no block reservation
	 * is necessary.
	 */
