...
{
	char			fname[256];
	struct xfs_name		xname = { .name = fname, .len = namelen };
	const char		*dstr = get_dstr(mp, dtype);
	xfs_dahash_t		hash;
	bool			good = libxfs_dir2_namecheck(name, namelen);

	memcpy(fname, name, namelen);
	fname[namelen] = 0;
	hash = libxfs_dir2_hashname(mp, &xname);

	dbprintf("%-18llu %-14s 0x%08llx %3d %s", ino, dstr, hash, namelen,
			fname);
	if (!good)
		dbprintf(_(" (corrupt)"));
	dbprintf("\n");
	...
	struct xfs_da_args		*args)
{
	...
	dir_emit(args->dp->i_mount, "..", 2, ino, XFS_DIR3_FT_DIR);
