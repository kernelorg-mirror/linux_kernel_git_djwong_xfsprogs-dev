	...
	struct xfs_inode		*ip)
{
	...
	for (i = 0; i < hdr->count; i++) {
		...
		if ((sfe->flags & XFS_ATTR_PARENT) &&
		    libxfs_parent_namecheck(sfe->flags, name, sfe->namelen) &&
		    libxfs_parent_valuecheck(mp, value, sfe->valuelen)) {
			libxfs_parent_irec_from_disk(&irec, name, sfe->namelen,
					value);
			pptr_emit(mp, &irec);
		}
	...
	struct xfs_buf			*bp)
{
	...
	for (i = 0; i < ichdr.count; entry++, i++) {
		...
		if (libxfs_parent_namecheck(entry->flags, name, namelen) &&
		    libxfs_parent_valuecheck(mp, value, valuelen)) {
			libxfs_parent_irec_from_disk(&irec, name, namelen,
					value);
			pptr_emit(mp, &irec);
		}
	}
	...
	xfs_ino_t		child_ino)
{
	...
	libxfs_parent_add(tp, ppargs, dp, &xname, ip);
