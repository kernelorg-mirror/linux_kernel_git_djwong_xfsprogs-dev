	...
	bool				is_meta)
{
	...
	struct remap_ent		*remap;
	unsigned char			*old_name = irec.p_name;
	xfs_dahash_t			hash;
	...
	if (!libxfs_parent_namecheck(attr_flags, name, namelen) ||
	 ...
	 * pointer.
	 */
	remap = remaptable_find(irec.p_ino, hash, name, namelen);
	if (remap) {
		remap_debug(
 "found obfuscated pptr 0x%lx '%.*s' -> 0x%lx -> '%.*s' \n",
				irec.p_ino, namelen, remap_ent_before(remap),
	...
	remap_debug("obfuscated pptr 0x%lx '%.*s' -> 0x%lx -> '%.*s'\n",
			irec.p_ino, namelen, old_name, metadump.cur_ino,
			namelen, name);
	if (!remaptable_add(irec.p_ino, hash, old_name, namelen, name))
		print_warning(
