	...
	void			*data)
{
	...
	error = -libxfs_alloc_read_agf(mp, tp, pag->pag_agno, 0, &agf_bp);
	if (error)
		goto out_cancel;

	fsbno = XFS_AGB_TO_FSB(mp, pag->pag_agno, start);
	error = -libxfs_free_extent(tp, fsbno, length, &XFS_RMAP_OINFO_AG,
			XFS_AG_RESV_AGFL);
	if (error)
		goto out_cancel;
	...
	int			quantity)
{
	...
	for (i = 0; i < quantity; i++) {
		...
		error = -libxfs_alloc_get_freelist(tp, agf_bp, &agbno, 0);
		if (error)
			goto out_cancel;
	...
	int			quantity)
{
	...
	error = -libxfs_alloc_read_agf(mp, tp, pag->pag_agno, 0, &targs.agbp);
	if (error)
		goto out_cancel;
	...
	error = -libxfs_alloc_read_agfl(mp, tp, pag->pag_agno, &agfl_bp);
	if (error)
		goto out_cancel;
	...
	for (i = 0; i < targs.len; i++) {
		error = -libxfs_alloc_put_freelist(tp, targs.agbp, agfl_bp,
				targs.agbno + i, 0);
		if (error)
			goto out_cancel;
	}
