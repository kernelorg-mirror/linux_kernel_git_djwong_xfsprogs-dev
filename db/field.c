...
/* attr3 specific fields */
	...
	{ FLDT_CCOWFLG, "ccowflag", fp_num, "%u", SI(REFCNTBT_COWFLAG_BITLEN), 0,
	  NULL, NULL },
	{ FLDT_RTCOWFLG, "rtcowflag", fp_num, "%u", SI(RTREFCNTBT_COWFLAG_BITLEN), 0,
	  NULL, NULL },
	...
	{ FLDT_RTREFCBTKEY, "rtrefcntbtkey", fp_sarray,
	  (char *)rtrefcbt_key_flds, SI(bitsz(struct xfs_rtrefcount_key)), 0,
	...
	{ FLDT_RTREFCBTREC, "rtrefcntbtrec", fp_sarray,
	  (char *)rtrefcbt_rec_flds, SI(bitsz(struct xfs_rtrefcount_rec)), 0,
...
/* dir v2/3 node fields */
	...
	{ FLDT_CDRFSBNO, "crfsbno", fp_num, "%llu", SI(bitsz(xfs_rfsblock_t) - 1),
	  FTARG_DONULL, fa_drfsbno, NULL },
