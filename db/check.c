	...
	xfs_rfsblock_t		*toti)
{
	...
	maxrecs = libxfs_rtrmapbt_maxrecs(XFS_DFORK_SIZE(dip, mp, whichfork),
			be16_to_cpu(dib->bb_level) == 0);
	...
	int			isroot)
{
	...
	if (xfs_sb_version_hassparseinodes(&mp->m_sb))
		blks_per_buf = mp->m_ino_geo.ig_blocks_per_cluster;
	else
		blks_per_buf = mp->m_ino_geo.ig_ialloc_blks;
	...
	if (level == 0) {
		if (be16_to_cpu(block->bb_numrecs) > mp->m_ino_geo.ig_inobt_mxr[0] ||
		    (isroot == 0 && be16_to_cpu(block->bb_numrecs) < mp->m_ino_geo.ig_inobt_mnr[0])) {
			dbprintf(_("bad btree nrecs (%u, min=%u, max=%u) in "
				 "inobt block %u/%u\n"),
				be16_to_cpu(block->bb_numrecs), mp->m_ino_geo.ig_inobt_mnr[0],
				mp->m_ino_geo.ig_inobt_mxr[0], seqno, bno);
			serious_error++;
			return;
		}
...
next_buf:
	...
	if (be16_to_cpu(block->bb_numrecs) > mp->m_ino_geo.ig_inobt_mxr[1] ||
	    (isroot == 0 && be16_to_cpu(block->bb_numrecs) < mp->m_ino_geo.ig_inobt_mnr[1])) {
		dbprintf(_("bad btree nrecs (%u, min=%u, max=%u) in inobt block "
			 "%u/%u\n"),
			be16_to_cpu(block->bb_numrecs), mp->m_ino_geo.ig_inobt_mnr[1],
			mp->m_ino_geo.ig_inobt_mxr[1], seqno, bno);
		serious_error++;
		return;
	}
	pp = XFS_INOBT_PTR_ADDR(mp, block, 1, mp->m_ino_geo.ig_inobt_mxr[1]);
	for (i = 0; i < be16_to_cpu(block->bb_numrecs); i++)
