...
typedef enum {
	...
	DBM_RLDATA,	DBM_COWDATA,	DBM_BTRTRMAP,	DBM_BTRTREFC,
	DBM_RLRTDATA,	DBM_NDBM
} dbm_t;
...
static xfs_ino_t	rrefcino = NULLFSINO;
	...
	char		**argv)
{
	...
	libxfs_imeta_lookup(mp, &XFS_IMETA_RTREFCOUNTBT, &rrefcino);
	...
	int			isfree)
{
	...
	static char		*fmtnames[] = {
		"dev", "local", "extents", "btree", "uuid", "rtrmap", "rtrefc"
	};
	...
	xfs_rfsblock_t		*toti)
{
	...
	if (be16_to_cpu(dib->bb_level) == 0) {
		...
		lastblock = 0;
		for (i = 0; i < be16_to_cpu(dib->bb_numrecs); i++) {
			if (be64_to_cpu(rp[i].rc_startblock) < lastblock) {
				dbprintf(_(
		"out-of-order rtrefc btree record %d (%u %u) root\n"),
					 i, be64_to_cpu(rp[i].rc_startblock),
					 be32_to_cpu(rp[i].rc_startblock));
			} else {
				lastblock = be64_to_cpu(rp[i].rc_startblock) +
					    be64_to_cpu(rp[i].rc_blockcount);
			}
		}
		return;
	} else {
	...
	if (level == 0) {
		...
		for (i = 0; i < be16_to_cpu(block->bb_numrecs); i++) {
			if (be32_to_cpu(rp[i].rc_refcount) == 1) {
				...
				bno = be64_to_cpu(rp[i].rc_startblock);
				if (bno >= XFS_RTREFC_COW_START) {
		...
		"leftover rt CoW extent at unexpected address (%lu) len %lu\n");
				...
				set_rdbmap(
					be64_to_cpu(rp[i].rc_startblock),
					be64_to_cpu(rp[i].rc_blockcount),
					DBM_RLRTDATA);
			}
			if (be64_to_cpu(rp[i].rc_startblock) < lastblock) {
