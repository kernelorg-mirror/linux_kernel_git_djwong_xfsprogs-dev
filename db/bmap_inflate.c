	...
	uint64_t		nr_blocks)
{
	...
	while (nr_blocks > 0) {
		...
		if (xfs_has_rmapbt(ip->i_mount)) {
			 ...
			 * records.
			 */
			tgt_agno = 1 + XFS_FSB_TO_AGNO(ip->i_mount,
					bd->irec.br_startblock);
			if (tgt_agno >= ip->i_mount->m_sb.sb_agcount)
				tgt_agno = 0;
			args.fsbno = XFS_AGB_TO_FSB(ip->i_mount, tgt_agno, 0);
		}
	...
	xfs_extnum_t			nextents)
{
	...
	ifake.if_whichfork = XFS_DATA_FORK;
