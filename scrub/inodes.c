	...
	struct xfs_bulkstat	*bstat)
{
	...
	for (i = 0, bs = bstat; i < XFS_INODES_PER_CHUNK; i++) {
		...
		/* Load the one inode. */
		req.hdr.ino = inogrp->xi_startino + i;
		error = xfrog_bulkstat_single(ctx->mnt_fd, &ctx->geo, &req);
		if (error || req.bulkstat.bs_ino != inogrp->xi_startino + i) {
			memset(bs, 0, sizeof(struct xfs_bulkstat));
			bs->bs_ino = inogrp->xi_startino + i;
			bs->bs_blksize = ctx->mnt_sv.f_frsize;
		} else
			memcpy(bs, &req.bulkstat, sizeof(*bs));
		bs++;
	}
}
	...
	void			*arg)
{
	...
	while ((error = xfrog_inumbers(ctx->mnt_fd, &ctx->geo, ireq)) == 0 &&
		...
		if (ireq->inumbers[0].xi_alloccount == 0)
			continue;
		breq->hdr.ino = ireq->inumbers[0].xi_startino;
		breq->hdr.icount = ireq->inumbers[0].xi_alloccount;
		error = xfrog_bulkstat(ctx->mnt_fd, &ctx->geo, breq);
		if (error)
			str_info(ctx, descr, "%s", strerror_r(errno,
						buf, DESCR_BUFSZ));

		xfs_iterate_inodes_range_check(ctx, ireq->inumbers,
				breq->bulkstat);
