	...
	void				*arg)
{
	...
	while (rv->io_length > 0) {
		...
		if (sz == len) {
			...
			/*
			 * A direct read hit an error before it could read any
			 * data at all.  Mark everything bad from io_start to
			 * the next miniosz block.  Move on to the next block,
			 * but only read a single block.
			 */
