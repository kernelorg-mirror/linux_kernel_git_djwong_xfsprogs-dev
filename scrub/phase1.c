	...
	struct scrub_ctx	*ctx)
{
	...
	disk_config_xfs_verify(ctx->datadev, ctx->mnt.fd,
			XFS_VERIFY_DATADEV);
	if (ctx->logdev)
		disk_config_xfs_verify(ctx->logdev, ctx->mnt.fd,
				XFS_VERIFY_LOGDEV);
	if (ctx->rtdev)
		disk_config_xfs_verify(ctx->rtdev, ctx->mnt.fd,
				XFS_VERIFY_RTDEV);
}
