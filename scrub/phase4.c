	...
	void				*priv)
{
	...
	while (!rls->aborted) {
		...
		ret = action_item_try_repair(ctx, &aitem, &made_progress);
		pthread_mutex_lock(&rls->lock);
	...
	if (atomic_dec_and_test(&rls->workers))
