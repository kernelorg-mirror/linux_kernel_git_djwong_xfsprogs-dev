	...
	char			**argv)
{
	...
	struct xfs_fsop_geom	*fsgeom = &frog->fsgeom;
	...
	if (optind != argc) {
		...
		offset = xfrog_bbtob(xfrog_agb_to_daddr(frog, agno, 0));
		length = xfrog_fsb_to_b(frog, fsgeom->agblocks);
	} else {
		offset = 0;
		length = xfrog_fsb_to_b(frog, fsgeom->datablocks);
	}
	...
	ret = ioctl(file->frog.fd, FITRIM, (unsigned long)&trim);
	if (ret < 0) {
