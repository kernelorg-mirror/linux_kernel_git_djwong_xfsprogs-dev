	...
	unsigned int			checked)
{
	...
	for (f = maps; f->mask != 0; f++) {
		if (f->has_fn && !f->has_fn(&file->frog.fsgeom))
			continue;
		if (!(checked & f->mask))
			continue;
		reported++;
	...
	xfs_agnumber_t		agno)
{
	...
	ret = xfrog_ag_geometry(file->frog.fd, agno, &ageo);
	if (ret) {
		perror("ag_geometry");
		return 1;
	}
	...
	const char		*descr)
{
	...
	ret = xfrog_bulkstat_single(&file->frog, ino, &bs);
	if (ret) {
		perror(descr);
		return 1;
	}
	...
	xfs_agnumber_t		agno)
{
	...
	if (agno != NULLAGNUMBER) {
		startino = xfrog_agino_to_ino(&file->frog, agno, 0);
		lastino = xfrog_agino_to_ino(&file->frog, agno + 1, 0) - 1;
	}

	while ((error = xfrog_bulkstat(&file->frog, &startino, 128, bstat,
			&ocount) == 0) && ocount > 0) {
		for (i = 0; i < ocount; i++) {
	...
	char			**argv)
{
	...
	if (file->frog.fsgeom.version != XFS_FSOP_GEOM_VERSION_V5) {
		perror("health");
		return 1;
	}
	...
	while ((c = getopt(argc, argv, "a:cfi:q")) != EOF) {
		...
		case 'f':
			...
			report_sick(_("filesystem"), fs_flags,
					file->frog.fsgeom.sick,
					file->frog.fsgeom.checked);
			if (comprehensive) {
	...
	if (default_report) {
		report_sick(_("filesystem"), fs_flags, file->frog.fsgeom.sick,
				file->frog.fsgeom.checked);

		for (agno = 0; agno < file->frog.fsgeom.agcount; agno++) {
			ret = report_ag_sick(agno);
			if (ret)
				return 1;
		}
