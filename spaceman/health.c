	...
	xfs_agnumber_t		agno)
{
	...
	ret = xfrog_ag_geometry(file->fd, agno, &ageo);
	if (ret) {
		perror("ag_geometry");
		return 1;
	}
	...
	const char		*descr)
{
	...
	ret = xfrog_bulkstat_single(file->fd, ino, &bs);
	if (ret) {
		perror(descr);
		return 1;
	}
	...
	long			agno)
{
	...
	while ((error = xfrog_bulkstat(file->fd, &startino, 128, bstat,
			&ocount) == 0) && ocount > 0) {
		for (i = 0; i < ocount; i++) {
