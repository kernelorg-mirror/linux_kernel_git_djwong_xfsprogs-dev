	...
	xfs_agnumber_t		agno)
{
	...
	if (agno != NULLAGNUMBER) {
		l->fmr_physical = xfrog_bbtob(
				xfrog_agb_to_daddr(frog, agno, 0));
		h->fmr_physical = xfrog_bbtob(
				xfrog_agb_to_daddr(frog, agno + 1, 0));
	...
	while (true) {
		ret = ioctl(file->frog.fd, FS_IOC_GETFSMAP, fsmap);
		if (ret < 0) {
		...
		for (i = 0, extent = fsmap->fmh_recs;
			...
			if (!(extent->fmr_flags & FMR_OF_SPECIAL_OWNER) ||
				...
				continue;
			agbno = xfrog_daddr_to_agbno(frog,
					xfrog_btobbt(extent->fmr_physical));
			aglen = xfrog_b_to_fsbt(frog, extent->fmr_length);
			freeblks += aglen;
			freeexts++;
...
init(
	...
	char			**argv)
{
	struct xfs_fsop_geom	*fsgeom = &file->frog.fsgeom;
	long long		x;
	int			c;
